...
class IndividualCheckboxWidget extends StatelessWidget {
  ...
  // Để thay thế TreeNodeWidget hoàn toàn
  ...
  // Để hiển thị độc lập như hiện tại
  ...
  // Để hiển thị với thông tin chi tiết
  ...
  // Tree view - thay thế TreeNodeWidget
  Widget _buildTreeView(BuildContext context, TreeNode node, GlobalTreeState state) {
    return Column(
      children: [
        _buildNodeRow(context, node, state),
        if (expandChildren && node.hasChildren)
          ...node.childKeys.map((childKey) => Padding(
  ...
  // Standalone view - như hiện tại
  Widget _buildStandaloneView(BuildContext context, TreeNode node, GlobalTreeState state) {
    return _buildNodeRow(context, node, state);
  }

  // Core node row widget
  Widget _buildNodeRow(BuildContext context, TreeNode node, GlobalTreeState state) {
    return Container(
        ...
        children: [
          Checkbox(
            ...
            onChanged: node.isEnabled && node.checkSource == CheckSource.manual
                ? (value) => context.read<GlobalTreeManager>()
                    .toggleNode(nodeKey, value ?? false)
                : null,
          ),
          Expanded(
            child: Column(
              ...
              children: [
                ...
                if (showRelationships) _buildRelationshipInfo(context, node, state),
              ],
            ),
          ),
        ],
  ...
  Widget _buildRelationshipInfo(BuildContext context, TreeNode node, GlobalTreeState state) {
